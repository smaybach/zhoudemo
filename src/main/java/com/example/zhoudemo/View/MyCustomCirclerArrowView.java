package com.example.zhoudemo.View;
...
public class MyCustomCirclerArrowView extends View {
    ...
    public MyCustomCirclerArrowView(Context context) {
        super(context);
        initVeiw(context);
    }


    public MyCustomCirclerArrowView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initVeiw(context);
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.MyCustomCirclerArrowView);
        for (int i = 0; i < typedArray.getIndexCount(); i++) {

            //就是我们自定义的属性的资源id
            int attr = typedArray.getIndex(i);
            switch (attr) {
                case R.styleable.MyCustomCirclerArrowView_circlr_bound_color:
                    circleBoundColor = typedArray.getColor(attr, Color.RED);
                    CurrenCircleBoundColor = circleBoundColor;
                    break;
                case R.styleable.MyCustomCirclerArrowView_circlr_bound_width:
